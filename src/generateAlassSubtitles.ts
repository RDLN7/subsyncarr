...
export async function generateAlassSubtitles(
  ...
  try {
    ...
    const command = `alass "${refPath}" "${srtPath}" "${outputPath}"`;
    const refSourceText = isSubRef
      ? isExtracted
        ? 'using extracted embedded sub'
        : `using subtitle ref: ${basename(refPath)}`
      : 'using video';
    console.log(`${new Date().toLocaleString()} Processing alass (${refSourceText}): ${command}`);
    ...
    try {
      ...
      // If direct video processing with alass failed, extract a 16kHz mono WAV audio reference file
      ...
      if (isVideoFile && existsSync(videoPath)) {
        ...
        console.log(`${new Date().toLocaleString()} Alass direct video processing failed. Extracting WAV audio reference to ${tempWavPath}...`);
        try {
          ...
          if (existsSync(tempWavPath)) {
            const wavAlassCmd = `alass "${tempWavPath}" "${srtPath}" "${outputPath}"`;
            console.log(`${new Date().toLocaleString()} Retrying alass with extracted WAV audio: ${wavAlassCmd}`);
            const res = await execPromise(wavAlassCmd, timeoutMs);
            stdout = res.stdout;
            stderr = res.stderr;
          } else {
            throw primaryError;
